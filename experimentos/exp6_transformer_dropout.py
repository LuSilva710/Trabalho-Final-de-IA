...
HEAD_DROPOUT_RATE = 0.5 # Dropout antes da camada final densa de classificação
...
# 3. Componentes da Arquitetura Vision Transformer (ViT) com Dropout
...
class PatchEncoder(layers.Layer):
    def __init__(self, num_patches, projection_dim, dropout_rate=0.0, **kwargs):
        super().__init__(**kwargs)
...
# 4. Construção do Modelo ViT com Dropout
def create_vit_classifier():
    inputs = layers.Input(shape=(IMG_HEIGHT, IMG_WIDTH, 3))
    
    # Gerar patches e codificá-los com embeddings de posição e dropout inicial
    patches = Patches(PATCH_SIZE)(inputs)
    encoded_patches = PatchEncoder(NUM_PATCHES, PROJECTION_DIM, dropout_rate=DROPOUT_RATE)(patches)
    ...
    for _ in range(TRANSFORMER_LAYERS):
        ...
        x3 = mlp(x3, hidden_units=TRANSFORMER_UNITS, dropout_rate=0.3) # Dropout interno do MLP
        x = layers.Add()([x3, x2])
        
    # Normalização e achatamento (Flatten) para classificação final
    representation = layers.LayerNormalization(epsilon=1e-6)(x)
    representation = layers.Flatten()(representation)
    representation = layers.Dropout(HEAD_DROPOUT_RATE)(representation) # Head Dropout
...
print("\nResumo da Arquitetura ViT com Dropout:")
model.summary()
...
print("\n--- RESULTADOS FINAIS DO EXPERIMENTO 6 ---")
