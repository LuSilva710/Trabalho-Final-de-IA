...
L2_REG = 0.01 # Coeficiente de regularização L2
...
# 3. Componentes da Arquitetura Vision Transformer (ViT) com Regularização L2
...
# 4. Construção do Modelo ViT com L2
def create_vit_classifier():
    inputs = layers.Input(shape=(IMG_HEIGHT, IMG_WIDTH, 3))
    
    # Gerar patches e codificá-los com embeddings e regularização L2 na projeção
    patches = Patches(PATCH_SIZE)(inputs)
    encoded_patches = PatchEncoder(NUM_PATCHES, PROJECTION_DIM, l2_reg=L2_REG)(patches)
    
    x = encoded_patches
    # Empilhar os blocos Transformer
    for _ in range(TRANSFORMER_LAYERS):
        # Primeiro sub-bloco: LayerNorm + MultiHeadAttention com L2 + Conexão Residual
        ...
        # Segundo sub-bloco: LayerNorm + MLP com L2 + Conexão Residual
        x3 = layers.LayerNormalization(epsilon=1e-6)(x2)
        x3 = mlp(x3, hidden_units=TRANSFORMER_UNITS, l2_reg=L2_REG)
        x = layers.Add()([x3, x2])
...
print("\nResumo da Arquitetura ViT com Regularização L2:")
model.summary()
...
print("\n--- RESULTADOS FINAIS DO EXPERIMENTO 7 ---")
